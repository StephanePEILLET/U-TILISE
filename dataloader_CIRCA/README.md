...
Comprendre comment est utilisé exactement l'arg masks_valid_obs
faire les stats sur les masks de nuages et voir si je peux sortir le nombre de date consécutive
Si je peux utiliser directement les fonctions de filtrage de U-TILISE en utilisant un fihcier hdf5
voir les différences avec notre propre fonction de filtrage

Commencer à faire le sampling de nuage de mask:
    - gestion des index des masks utilisés / réutilisés ?
    - réfléchir d'abord à la manière aléatoire
    - réfléchir à la manière déterministique
    - en fonctoin des arguments pour les masks, regarder s'il faut de l'overlap où non 
    - faut-il se limiter au nuages présent dans une zone, comment faire les répartitions de nuages
