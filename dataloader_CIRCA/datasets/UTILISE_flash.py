...
class UTILISE_Flash(CircaPatchDataSet):
    ...
    def setup_filter_settings(self, filter_settings: Optional[DictConfig] = None, max_seq_length: Optional[int] = None):
        if filter_settings is None:
            self.filter_settings = {'type': None, 'min_length': 5, 'return_valid_obs_only': False, 'max_t_sampling': None}
    ...
    def load_data_from_files(self, item: int):
        ...
        return {
            ...
            "valid_obs": np.asarray([1 if i in idx_good_frames else 0 for i in range(len(dates_S2))]),
        }

    def _longest_consecutive_seq_within_sampling_frequency(self, sample: dict) -> Dict[str, int]:
        """
    ...
    def _subsample_sequence(self, sample: dict, seq_length: int) -> Tuple[torch.Tensor, torch.Tensor]:
        """
    ...
    def __getitem__(self, item: int, t_sampled: Optional[Tensor] = None, t_masked: Optional[Dict[str, np.ndarray]] = None):
        ...
        cloud_mask = torch.from_numpy(patch['S2']['cloud_mask']) #.squeeze(1) # Load the cloud masks, T x 1 x H x W => T x H x W

        # Temporally subsample/trim the sequence
        if t_sampled is None:
            t_sampled, masks_valid_obs = self._subsample_sequence(patch, seq_length=s2.shape[0])
        else:
            masks_valid_obs = torch.ones(len(t_sampled), )

        frames_input, frames_target = s2[t_sampled].clone(), s2[t_sampled].clone()
        s2_dates = patch['S2']['S2_dates'][t_sampled]  # Extract the acquisition dates of the temporally trimmed S2 image sequence
        ...
        out = {
            ...
            'cloud_prob': torch.from_numpy(patch['S2']['cloud_prob'].astype(np.float32)),
            'cloud_mask': cloud_mask,
        }
        if self.use_SAR:
...
######################################################################################
    ...
    def _generate_masks(
        ...
        if self.mask_kwargs.mask_type == 'random_clouds':
            ...
            sampled_clouds = self._sample_cloud_masks_from_tiles(id_obs, sample, len(t_masked['indices_masked']))
