...
def str2ActivationType(
    activation: str | Tuple[str, float],
) -> ActivationType | Tuple[ActivationType, float]:
...
def get_activation(
    activation: ActivationType | Tuple[ActivationType, float] | str | Tuple[str, float],
):
    """
    ...
    if isinstance(activation, str) or (
        isinstance(activation, tuple) and isinstance(activation[0], str)
    ):
        # Conversion:
    ...
    if isinstance(activation, tuple) and activation[0] == ActivationType.LRELU:
        return (
            nn.LeakyReLU(activation[1])
            if isinstance(activation[1], float)
...
def get_group_gn(dim, dim_per_gp, num_groups):
    ...
    assert (dim_per_gp == -1 and num_groups != -1) or (
        dim_per_gp != -1 and num_groups == -1
