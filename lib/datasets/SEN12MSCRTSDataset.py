...
class SEN12MSCRTSDataset(EarthNet2021Dataset):
    ...
    def __init__(
        ...
        if (
            ...
            or (
                "max_t_sampling" in filter_settings
                and (
                    not isinstance(filter_settings.max_t_sampling, int)
                    and filter_settings.max_t_sampling is not None
                )
            )
        ):
            raise RuntimeError(
        ...
        if not isinstance(return_acquisition_dates, bool):
            raise TypeError(
                "Invalid `return_acquisition_dates` parameter. Specify a boolean."
            )
        ...
        # Image size
        self.image_size = (
            crop_settings.shape if self.crop_settings.enabled else (256, 256)
        )
    ...
    def __getitem__(
        ...
        if not self.preprocessed or self.split == "train":
            ...
            data = torch.from_numpy(
                patch["S2/S2"][:, self.s2_channels, :, :].astype(np.float32)
            )

            # Load the cloud masks, T x 1 x H x W
            data = torch.cat(
                (data, torch.from_numpy(patch["S2/cloud_mask"][:]).float()), dim=1
            )

            if self.return_cloud_prob:
                # Load the cloud probability maps, T x 1 x H x W
                data = torch.cat(
                    (data, torch.from_numpy(patch["S2/cloud_prob"][:])), dim=1
                )
            ...
            # probability maps
            cloud_mask = data[:, len(self.s2_channels), :, :].unsqueeze(1)[
                t_sampled, :, :, :
            ]
            if self.return_cloud_prob:
                cc = data[:, len(self.s2_channels) + 1, :, :].unsqueeze(1)[
                    t_sampled, :, :, :
                ]
            ...
            s2_dates = [
                dataset_tools.str2date(date.decode("utf-8"))
                for date in patch["S2/S2_dates"]
            ]
            ...
            if self.mask_kwargs is not None:
                t_masked, frames_input, masks = self._generate_masks(
                    patch, frames_input, cloud_mask, t_masked
                )
            else:
                masks = torch.zeros(
            ...
            s2_dates = [
                dataset_tools.str2date(date.decode("utf-8"))
                for date in sample["S2/S2_dates"]
            ]
            ...
            if self.include_S1:
                ...
                s1_dates = [
                    dataset_tools.str2date(date.decode("utf-8"))
                    for date in sample["S1/S1_dates"]
                ]
    ...
    def _get_data_samples(self) -> Tuple[List[str], Dict[str, List[str]]]:
        ...
        if (
            ...
            and self.filter_settings.max_t_sampling is not None
            and self.ignore_seq_with_black_images
        ):
        ...
        if not skip_parsing:
            if (
                self.filter_settings is not None
                and self.filter_settings.type == "cloud-free"
                and "path_samples" in self.f.keys()
            ):
                ...
                if (
                    self.filter_settings.max_t_sampling is not None
            ...
            if self.ignore_seq_with_black_images:
                ...
                for p in paths:
                    ...
                    if torch.any(s2.sum(dim=-1).sum(dim=-1).sum(dim=-1) == 0.0):
                        invalid.append(p)
                for p in invalid:
                    paths.remove(p)
    ...
    def _sample_cloud_masks_from_tiles(
        ...
        while len(cloud_mask) < n:
            ...
            seq = torch.from_numpy(
                self.f[random.choice(samples)]["S2/cloud_mask"][:]
            ).float()
        ...
        if isinstance(sample, h5py.Group) and "S2" in sample.keys():
            ...
            if self.filter_settings.type == "cloud-free":
                ...
                if seq_length >= self.filter_settings.min_length:
                    ...
                    if self.filter_settings.max_t_sampling is not None:
                        count = self._longest_consecutive_seq_within_sampling_frequency(
                            sample
                        )["len"]
                        ...
                        if self.verbose == 1:
                            print(
                                f"Less than {self.filter_settings.min_length} consecutive cloud-free frames with "
    ...
    def _longest_consecutive_seq_within_sampling_frequency(
        self, sample: h5py._hl.group.Group
    ) -> Dict[str, int]:
        """
